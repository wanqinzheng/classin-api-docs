...
| studentName | 否 | 1-24 字，不区分中英文，超过 24 会自动截取为 24 字 | 机构后台旁听生的姓名 | 仅用于当 identity 为 2（旁听身份）时，才使用此参数。当 identity 为 2 时，如果没有传此参的话，则使用手机号码作为旁听生的名字 |
| addCourseStudentReplay | 否 | 0或1 | 是否允许该学生观看历史活动（包括历史课堂回放）注：仅支持学生，旁听不支持，传参不报错 |当identity为1（学生）时，才使用此参数。 0表示不允许，1表示允许，不传默认走系统设置 |
