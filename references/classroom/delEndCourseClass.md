# 删除已结课节
删除已结课节，需要 SID、safekey、timeStamp、课程 ID 和课节 ID 等，返回删除后的说明。

注： 如果有课节唯一标识，则课节删除成功后会将课节唯一标识一起删除。

## URL

`https://root_url/partner/api/course.api.php?action=delEndCourseClass`
