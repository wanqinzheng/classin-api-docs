# 修改机构标签
修改机构标签，需要 SID，safekey，timeStamp 标签 ID，标签名称。返回执行后的说明。

## URL

`https://root_url/partner/api/course.api.php?action=updateSchoolLabel`  
...
| labelId | 是 | 无 | 标签 ID | 无|
| labelName | 是 | 1-20个字，不区分中英文，超过20个字会自动截取为20字 | 标签名称 | 无|
