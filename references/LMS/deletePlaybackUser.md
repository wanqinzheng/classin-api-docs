...
此为 LMS 课堂活动接口，用于为课堂活动删除客户端回放可观看人员。 
...
1. 关于本接口--删除客户端回放可观看人员
   - 删除可观看人员，每个人员有单独的返回码，返回码在data里。
   - 如果删除的成员不在可观看列表中，不会返回错误码，会返回在成功列表中，正常执行
...
`https://root_url/playback/deletePlaybackUser`
...
| activityId | 否 | integer  | 课堂活动ID  | 和课节id二选一，如果有活动id优先使用活动id |
| classId | 否 | integer | 课节ID | 和活动id二选一，如果有活动id优先使用活动id |
...
| └ successList | array[int] |  | 删除成功的人员uid |
| └ failedList | array[object] | 1 | 删除失败的人员uid和错误信息 |
|     └ uid | int |  | 人员uid |
|     └ code | int |  | 错误码 |
|     └ msg | string |  | 错误信息 |
...
```http
POST /playback/deletePlaybackUser HTTP/1.1
Host: root_url
